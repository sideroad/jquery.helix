(function( $ ){
    ...
    var refreshClass = function($children, data){
            ...
            $children.filter(':lt('+parseInt(data.devide*data.turns, 10)+')').addClass('show');
            $children.filter(':first').addClass('first');
            $children.filter(':eq('+(data.half)+')').addClass('focus');
            $children.filter(':eq('+parseInt((data.devide*data.turns)-1, 10)+')').addClass('last');
        },
        initialize = function($elem){
            var children = $elem.addClass("helix").children(),
                ...
                cnt = 0,
                half = parseInt( (devide*options.turns)/2, 10 ),
            ...
            children.filter(':gt('+parseInt(length - half -1+(length%2), 10)+')').prependTo($elem);

            children = $elem.children();
            children.each(function(){
                    var $this = $(this);
                    $this.css({
                             'transform': 'rotateY( '+parseInt( deg, 10 )+'deg ) translateZ( '+translateZ+'px )'
                         })
                         .transition({
                             y: start + (cnt*ramp)
                         });
            ...
            $elem.wrap($container);
            $elem.css({
                     transform: 'translateY(0px) translateZ( -'+translateZ+'px ) rotateY('+parseInt(rotateY*-1, 10)+'deg)',
                     width: options.item.width
                 })
                 .show()
                 .data(data);
        ...
        move = function($elem, direction){
            ...
            $elem.css('transform', 'translateY('+(first*ramp*-1)+'px) translateZ( -'+translateZ+'px ) rotateY('+parseInt( rotateY*-1, 10 )+'deg)')
                 .data({
                     rotateY: rotateY,
                     children : children,
                     focus: focus,
